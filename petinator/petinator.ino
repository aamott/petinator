...
void check_thermal_safety()
{
  ...
  if (current_temp < target_temp - TEMP_VARIANCE && cooling)
  { // if below temp but not heating
    // set to heating mode
    cooling = false;
    // reset the temps array
    reset_recorded_temps();
  }
  else if (current_temp > target_temp + TEMP_VARIANCE && !cooling)
  { // if above temp but not cooling
    // set to cooling mode
    cooling = true;
    // reset the temps array
    reset_recorded_temps();
  ...
  if (heatingEnabled)
  {
    ...
    if (millis() - last_thermal_check_time > TEMP_CHECK_MS && // time to check again
        (current_temp < target_temp - TEMP_VARIANCE ||        // heating
         current_temp > target_temp + TEMP_VARIANCE))
    { // cooling
      last_thermal_check_time = millis();

      // update PID
      if (temp_set_point - BANG_BANG_LOWER > current_temp)
      { // bang-bang for low temps
        analogWrite(HEATER_PIN, 255);
        heaterPID.clear(); // reset PID
      }
      else if (temp_set_point + BANG_BANG_UPPER < current_temp)
      { // bang-bang for high temps
